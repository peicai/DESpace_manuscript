...
### Melanoma
load("~/Desktop/master_thesis/Paper/Data/Real/melanoma/mel1_rep1_melanoma.rda")
...
### mouse cerebellum
load("~/Desktop/master_thesis/Paper/Data/Real/slideseq2/cerebellum_filtered_100.rda")
CD <- colData(sce_one)
cell_ID = rownames(colData(sce_one))
colData(sce_one) <- cbind(CD, cell_ID)
library(readr)
stLearn_results <- read_csv("~/Desktop/master_thesis/Paper/Data/Real/slideseq2/100_stLearn_results.csv")
...
# (mouse_cluster_plot <- clusterPlot(
#   mouse_sce,
#   label = "stLearn_pca_kmeans",
#   color = NA,
#   platform = "Visium",
#   is.enhanced = TRUE)+ scale_y_reverse()+ scale_x_reverse())
...
plot3 <- grid.arrange(mouse_cluster_plot + labs(title = "mouse cerebellum") + 
                      ...
                      legend, nrow = 2, heights = c(7, 1))
AA <- grid.arrange(plot1,plot2,plot3, ncol = 3, widths = c(1,1,1))

# (AA = egg::ggarrange( plots = list(LIBD_cluster_plot + labs(title = "LIBD") + theme(plot.title = element_text(hjust = 0.5, face = "bold", size=10)) + theme(legend.position="bottom"),
#                                    melanoma_cluster_plot +  labs(title = "melanoma",labs(color = "")) + theme(plot.title = element_text(hjust = 0.5, face = "bold", size=10))+ theme(legend.position="bottom"),
#                                    mouse_cluster_plot +  labs(title = "mouse cerebellum")+ theme(plot.title = element_text(hjust = 0.5, face = "bold", size=10)+ theme(legend.position="bottom"))
# ),
# ncol = 3, nrow = 1))
path_save = "/Users/peiyingcai/Desktop/master_thesis/Paper/Figures/Real/"
ggsave(filename = paste0(path_save,"Cluster_plot.pdf"),
       plot = AA,
       device = "pdf",
       width = 10,
       height = 4.5,
