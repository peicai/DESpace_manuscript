...
library(lubridate)
library(ggpattern)
path = "/Users/peiyingcai/Desktop/master_thesis/Paper/Data/Real/"
path_save = "/Users/peiyingcai/Desktop/master_thesis/Paper/Figures/Real/"
source("~/Desktop/master_thesis/Paper/Scripts/Plots_Function.R")
source("~/Desktop/master_thesis/Paper/Scripts/All_methods.R")
color_bar = c("#A6CEE3","#1F78B4","#08306B","#FFD92F",
...
`%notin%` <- Negate(`%in%`)
d3$DESpace <- as.factor(ifelse(d3$Method_sub %in% c('DESpace','Manual_DESpace'), 'DESpace', 'Cluster'))  
...
(gg_Time_LIBD <- d3_LIBD %>%
    ggplot(aes(x = Method, Time_Minutes, fill = color_all, pattern = DESpace)) +
    ...
    scale_pattern_manual(values = c(DESpace = "stripe", Cluster = "none"),
                         guide = "none") +
...
sample_names = c("mel1_rep1", "mel1_rep2",
                 ...
                 "mel3_rep1", "mel3_rep2",
                 "mel4_rep1", "mel4_rep2")
computational_cost <- read_csv("~/Desktop/master_thesis/Paper/Data/Real/melanoma/computational_cost.csv")
setwd("~/Desktop/master_thesis/Paper/Data/Real/melanoma")
df <- computational_cost
...
`%notin%` <- Negate(`%in%`)
d3$DESpace <- as.factor(ifelse(d3$Method_sub %in% c('DESpace'), 'DESpace', 'Cluster'))
d3 %>% mutate(highligth = case_when(d3$Method_sub %in% c('BayesSpace','StLearn') ~ "yes",
                                    TRUE ~ "no"))
#remotes::install_github("coolbutuseless/ggpattern")
d3$Time = c(Time, NA, NA)
...
computational_cost1 <- read_csv("~/Desktop/master_thesis/Paper/Data/Real/slideseq2/computational_cost1.csv")
computational_cost1$...1 <- c("none","BayesSpace_DESpace","none","SPARK-X", "MERINGUE")
computational_cost2 <- read_csv("~/Desktop/master_thesis/Paper/Data/Real/slideseq2/computational_cost2.csv")
df <- as.data.frame(computational_cost2)
computational_cost_stLearn_edgeR <- read_csv("~/Desktop/master_thesis/Paper/Data/Real/slideseq2/computational_cost_stLearn_edgeR.csv")
colnames(df)[1] <- "times"
...
setwd("~/Desktop/master_thesis/Paper/Data/Real/slideseq2")
...
(AA = egg::ggarrange( plots = list(gg_Time_LIBD + labs(title = "LIBD") + theme(plot.title = element_text(hjust = 0.5, face = "bold", size=15)),
                                   gg_Time_melanoma +  labs(title = "melanoma") + theme(plot.title = element_text(hjust = 0.5, face = "bold", size=15)),
                                   gg_Time_mouse +  labs(title = "mouse cerebellum")+ theme(plot.title = element_text(hjust = 0.5, face = "bold", size=15))
),
