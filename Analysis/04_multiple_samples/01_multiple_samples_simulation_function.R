...
clusters_BayesSpace <- function(sce, q, platform){
  print("Start clustering (BayesSpace)...")
  set.seed(123)
...
JointClus_BayesSpace <- function(sce.combined, q, path,sample_names){
  set.seed(123)
  colData(sce.combined)$row <- colData(sce.combined)$imagerow
  colData(sce.combined)$col <- colData(sce.combined)$imagecol
  sce.combined = spatialPreprocess(sce.combined, n.PCs = 50) #lognormalize, PCA
  
  sce.combined = scater::runUMAP(sce.combined, dimred = "PCA")
  ...
  sce.combined = harmony::RunHarmony(sce.combined, "sample_name", verbose = T,theta = 5, lambda = 0.1)
  sce.combined = scater::runUMAP(sce.combined, dimred = "HARMONY", name = "UMAP.HARMONY")
  colnames(reducedDim(sce.combined, "UMAP.HARMONY")) = c("UMAP1", "UMAP2")
  

  sce.combined$col <- as.numeric(as.character(sce.combined$col))
  sce.combined$row <- as.numeric(as.character(sce.combined$row))
  sce.combined$col[sce.combined$sample_name == "mel1_rep1"] = 
    30 + sce.combined$col[sce.combined$sample_name == "mel1_rep1"]
  sce.combined$col[sce.combined$sample_name == "mel2_rep1"] = 
    30 + sce.combined$col[sce.combined$sample_name == "mel2_rep1"]
  sce.combined$row[sce.combined$sample_name == "mel2_rep1"] = 
    35 + sce.combined$row[sce.combined$sample_name == "mel2_rep1"]
  
  sce.combined$row[sce.combined$sample_name == "mel3_rep1"] = 
    0 + sce.combined$row[sce.combined$sample_name == "mel3_rep1"]
  sce.combined$row[sce.combined$sample_name == "mel4_rep1"] = 
    30 + sce.combined$row[sce.combined$sample_name == "mel4_rep1"]
  sce.combined$col[sce.combined$sample_name == "mel4_rep1"] = 
    10 + sce.combined$col[sce.combined$sample_name == "mel4_rep1"]
  
  
  (g3 <- clusterPlot(sce.combined, "sample_name", color = NA) + #make sure no overlap between samples
      labs(fill = "Sample", title = "Offset check"))
  ggsave(paste0(path, "Offset_check.png"))
  #dev.off()
  
  sce.combined = spatialCluster(sce.combined, use.dimred = "HARMONY", q=q) #use HARMONY
  (g4 <- clusterPlot(sce.combined, color = NA) + #plot clusters
      labs(title = "BayesSpace joint clustering"))
  ggsave(paste0(path, "BayesSpace_clusters.png"))
  #dev.off()
  return(sce.combined)
}
...
SV_edgeR_counts = function(sce_object,
  ...
  expr = assays(sce_object, withDimnames=FALSE)$counts
  ...
  if(default == T){
    ...
    if(pattern_name %in% c('circle_patch','right_patch','bottom_patch')){q=2}
  ...
  if(cluster_method == 'BayesSpace'){save(sce.combined, paste0(dir,"/sce_combined_BayesSpace.rda"))
  ...
  y <- DGEList(counts=assays(sce_object, withDimnames=FALSE)$counts, 
               genes=rownames(assays(sce_object, withDimnames=FALSE)$counts))
...
Single_multi_edgeR = function(sce_object,
                              #min_count = 50,
                              #coordinate_name = c("row", "col"),
                              #num_core = 4,
                              #prior.count = 10, # edgeR:cpm
                              ...
                              dir = "~",
                              #pattern_name = 'Manual_clusters_patch',
                              #cluster_method='multi_BayesSpace',
                              edgeR_method = 'multi',
  ...
  if('multi' %in% edgeR_method){
    message("using 'multi_edgeR' for spatial gene/pattern detection. ")
    
    layer = factor(colData(sce_object)[[layer_names]] )
    layer = droplevels(layer)
    sce_object = sce_object[, !is.na(layer)]
    design = data.frame(condition = factor(layer),
                        sample_id = factor(colData(sce_object)$sample_name))
    ...
    y <- estimateDisp(y, design_model, #robust=TRUE, 
                      BPPARAM = BPPARAM)
    fit <- glmFit(y, design_model, BPPARAM = BPPARAM)
    q = nlevels(factor(colData(sce_object)[[layer_names]]))
    print(paste0("num clusters: ", q))
    ...
    DT_results[, genes := gene_names]
    DT_results[, sample := ""]
    DT_results[, method := 'SV_multi_edgeR']
    DT_results[, cluster := cluster_name]
    DT_results = DT_results[,.(genes, logCPM, LR, PValue, FDR, sample, method, cluster)]
    data.table::setorder(DT_results, FDR, PValue)
    data.table::setorder(DT_results, FDR, PValue)
    DT_results_multi = DT_results
  }else{DT_results_multi = data.table()}
  
  if('single' %in% edgeR_method){
    message("using 'single' for spatial gene/pattern detection. ")
    
    sample_names = unique(colData(sce_object)$sample_name)
    n_sample = length(sample_names)
    results = data.frame()
    results = lapply(1:n_sample,single_edgeR,
                     sce_object=sce_object,
                     sample_names=sample_names,
                     layer_names=layer_names,
                     cluster_name=cluster_name)
    
    DT_results1 = do.call("rbind", results)
  }else{DT_results1 = data.table()}
  ## extract expression values from gobject
  
  DT_results2 = rbind(DT_results_multi, DT_results1)
  ## return results ##
  
  return(DT_results2)
}
single_edgeR <- function(i,sce_object,sample_names,layer_names,cluster_name){
  set.seed(123)
  ...
  design$condition <- droplevels(design$condition)
  y <- DGEList(counts=assays(sce, withDimnames=FALSE)$counts, 
               genes=rownames(assays(sce, withDimnames=FALSE)$counts))
  y$samples$lib.size <- colSums(y$counts)
  y <- calcNormFactors(y)
  design_model <- model.matrix(~design$condition)
  rownames(design_model) <- colnames(y)
  ...
  y <- estimateDisp(y, design_model, #robust=TRUE, 
                    BPPARAM = BPPARAM)
  fit <- glmFit(y, design_model, BPPARAM = BPPARAM)
  q = nlevels(factor(colData(sce)[[layer_names]]))
  print(paste0("num clusters: ", q))
  
  lrt <- glmLRT(fit, coef = 2:q)
  res_edgeR = topTags(lrt, n = Inf)
  
  DT_results1 = data.table::as.data.table(res_edgeR[[1]])
  gene_names = rownames(res_edgeR[[1]])
  DT_results1[, genes := gene_names]
  DT_results1[, sample := sample_names[i]]
  DT_results1[, method := 'SV_single_edgeR']
  DT_results1[, cluster := cluster_name]
  DT_results1 = DT_results1[,.(genes, logCPM, LR, PValue, FDR, sample, method, cluster)]
  data.table::setorder(DT_results1, FDR, PValue)
  #DT_results = rbind(DT_results, DT_results1)
  return(DT_results1)
}
...
#' @keywords internal
CombineSimulatedObject = function(sce_list,
                                  pattern_name = 'pattern',
                                  SvGene,
                                  sample_names,
                                  coordinate_name = c("row", "col"),
                                  pattern_cell_ids_all = NULL,
                                  gene_name = NULL,
                                  ...
                                  ...) {
  set.seed(123)
  if(is.null(pattern_cell_ids_all)) {
    stop('pattern_cell_ids_all can not be NULL \n')
  }
  pb <- txtProgressBar(min = 0, max = length(gene_name), style = 3,
                       width = 50,   # Progress bar width. Defaults to getOption("width")
                       char = "=")
  prob_list = list()
  SimCombined_list <- list()
  ## First 3 easy patterns
  if(length(spatial_probs) == 2){
    # The number of genes of "3" sce objects in the sce_list are exactly the same; use the first sce object's dim to split SV genes into 2 part: high_SVG and low_SVG
    sce_object = sce_list[[1]]
    ...
    ## Save grund truth for each sample: "in" or "out" cell ids
    GroudTruth = list()
    #GroudTruth = replicate(length(sce_list), GroudTruth, simplify = FALSE)
    for (ll in c(1:length(sce_list))){
      GroudTruth[[ll]] = data.frame(matrix(ncol = 3, nrow = dim(simobject_list[[ll]])[2]))
      colnames(GroudTruth[[ll]]) = c("high_SVG", "low_SVG", "non_SVG")
      cell_meta_original = data.table::as.data.table(colData(simobject_list[[ll]]))
      GroudTruth[[ll]]$high_SVG <- with(cell_meta_original,ifelse(cell_ID %in% pattern_cell_ids_all[[ll]], 'in', 'out'))
      GroudTruth[[ll]]$non_SVG <- with(cell_meta_original,ifelse(cell_ID %in% pattern_cell_ids_all[[ll]], 'in', 'out'))
      GroudTruth[[ll]]$low_SVG <- with(cell_meta_original,ifelse(cell_ID %in% pattern_cell_ids_all[[ll]], 'out', 'in'))
    }
    
    save(GroudTruth,file = paste0(resultdir, 'Ground_Truth.rda'))
    
    #foreach(ll = c(1:length(sce_list))) %dopar% {
    for(ll in c(1:length(sce_list))){
      final_object = simobject = simobject_list[[ll]]
      for (p in c(1:2)){
        prob_i = spatial_probs[p]
        gene_list = gene_split[[p]]
        if(verbose) cat('\n \n start with the sample', ll, '\n \n')
        if(verbose) cat('\n \n start with ', prob_i, '\n \n')
        ## if(verbose) cat('\n \n repetition = ', rep_i, '\n \n')
        print(length(gene_name))
        
        for(gene_ind in 1:length(gene_name)) {
          gene = gene_name[gene_ind]
          #print(gene)
          #print(paste0(round(gene_ind/length(gene_name)*100),"%"))
          # Updates the current state
          setTxtProgressBar(pb, gene_ind)
          plot_name = paste0('plot_',gene,'_prob', prob_i, '_rep', rep_i)
          cell_meta_original = data.table::as.data.table(colData(simobject))
          ## create and add annotation for pattern
          if(gene %in% c(non_SVG, high_SVG)){
            cell_meta_original[, (pattern_name) := ifelse(cell_ID %in% pattern_cell_ids_all[[ll]], 'in', 'out')]
          }else if(gene %in% low_SVG){
            cell_meta_original[, (pattern_name) := ifelse(cell_ID %in% pattern_cell_ids_all[[ll]], 'out', 'in')]
          }
          
          gg = simulateOneGenePatternObject(sce_object = simobject,
        ...
        close(pb)
        #if(sample_names[ll] != unique(colData(final_object1)$sample_name)){
        #   stop("Wrong sample name!!")
        # }
        print(sample_names[ll])
        if(save_raw == TRUE) {
          ...
          write.table(x = as.matrix(counts(gg)),
                      file = paste0(resultdir,  save_name,'_',prob_i, '_raw_data_',sample_names[ll],'.txt'),
                      sep = '\t')
          save(gg, file = paste0(resultdir, save_name,'_',prob_i,'_final_object_',sample_names[ll],'.rda'))
      ...
      write.table(x = as.matrix(assays(final_object)$counts),
                  file = paste0(resultdir, save_name,'_',spatial_probs[1],'_',spatial_probs[2],'_final_raw_data',sample_names[ll],'.txt'),
                  sep = '\t')
      
      save(final_object, file = paste0(resultdir, save_name,'_',spatial_probs[1],'_',spatial_probs[2],'_final_object',sample_names[ll],'.rda'))
      SimCombined_list[[ll]] <- final_object
    } # end foreach ll
    SimCombined_sce <- do.call(cbind, SimCombined_list)
    final_object <- SimCombined_sce
    save(final_object, file = paste0(resultdir, save_name,'_',spatial_probs[1],'_',spatial_probs[2],'_final_combined_object.rda'))
    
    return(SimCombined_sce)
  }else {
    ## Mixture pattern and inverted mixture pattern
    #for(ll in 1:length(sce_list)){
    registerDoParallel(cores = 3)
    foreach(ll = 1:length(sce_list))%dopar%{
      combined_object = sce_list[[ll]]
      sce_object = sce_list[[ll]]
      ## Save grund truth for each sample: "in" or "out" cell ids
      GroudTruth = list()
      #GroudTruth = replicate(length(sce_list), GroudTruth, simplify = FALSE)
      for (ig in c(1:length(sce_list))){
        GroudTruth[[ig]] = data.frame(matrix(ncol = length(spatial_probs), nrow = dim(sce_object)[2]))
        colnames(GroudTruth[[ig]]) = paste0(spatial_probs,"_",seq(spatial_probs))
      } # end for
      
      for(i in c(1:length(spatial_probs))){
        simobject = sce_list[[ll]]
        prob_i = spatial_probs[i]
        gene_list = SvGene[[i]]
        if(verbose) cat('\n \n start with ', prob_i, '_',i,'\n \n')
        cell_meta_original = data.table::as.data.table(colData(sce_object))
        pattern_cell_ids = pattern_cell_ids_all[[ll]]
        if(spatial_probs[1] == 0.5){
          ifelse(i==1, cell_ids <-  colnames(assays(sce_object)$counts), cell_ids <-  pattern_cell_ids[[i-1]])
        }else { ifelse(i==1, cell_ids <-  colnames(assays(sce_object)$counts), cell_ids <- pattern_cell_ids[[i]])} # end ifelse
        ...
        GroudTruth[[ll]][,i] <- ifelse(cell_meta_original$cell_ID %in% cell_ids, 'in', 'out')
        ...
        for(gene_ind in 1:length(gene_list)) {
          ...
          setTxtProgressBar(pb, gene_ind)
          plot_name = paste0('plot_',gene,'_prob', prob_i, '_rep', rep_i)
          #assign(paste0("final_gobject_",i), 
          gg <-   simulateOneGenePatternObject(sce_object = sce_object, # sce_list[[ll]],
                                               coordinate_name = coordinate_name,
                                               pattern_name = pattern_name,
                                               cell_meta = cell_meta_original,
        ...
        subdir = paste0(save_dir,'/',pattern_name,'/')
        resultdir <- paste0(subdir,'probs_',spatial_probs[1],'_',spatial_probs[2],default,'/')
        if(!file.exists(folder_path)) dir.create(resultdir, recursive = TRUE)
        write.table(x = as.matrix(assays(gg)$counts),
                    file = paste0(resultdir,  save_name,'_',prob_i, '_',i,'_raw_data',sample_names[ll],'.txt'),
                    sep = '\t')
        save(gg, file = paste0(resultdir, save_name,'_',prob_i,'_',i,'_final_object',sample_names[ll],'.rda'))
      ...
      } # end for i in 1:length(spatial_probs)
      ...
      save(combined_object, file = paste0(resultdir, save_name,'_',spatial_probs[1],'_',spatial_probs[2],'_final_object',sample_names[ll],'.rda'))
      SimCombined_list[[ll]] <- combined_object
    } # end foreach ll in 1:length(sce_list)
    SimCombined_sce <- do.call(cbind, SimCombined_list)
    combined_object <- SimCombined_sce
    save(combined_object, file = paste0(resultdir, save_name,'_',spatial_probs[1],'_',spatial_probs[2],'_final_combined_object.rda'))
    save(GroudTruth,file = paste0(resultdir, 'Ground_Truth.rda'))
...
run_spatial_sim_tests_methods = function(sce_object = final_object,raw_sce = raw_object,rep_i = rep_i,
  ...
  if(run_simulations == TRUE) {
    ...
    for(test in 1:length(spat_methods)) {
      ...
      print("Run spatial analysis")
      if(selected_method == 'SV_edgeR_counts') 
      {
        print('Method: SV_edgeR_counts')
        ## spark
        start = proc.time()
...
#' @export
runPatternSimulation = function(sce_list, rawobject = object_subset,
                                coordinate_name = coordinate_name,
                                SvGene,
                                sample_names,
                                pattern_name = 'pattern',
  ...
  if(is.null(cluster_index)){
    ...
    if(spatial_probs[1] != 0.5 &&
       length(spatial_probs) != length(SV_percent) )
    {stop('lengths of spatial probabilities, SV_percent and number of spatial mix patterns need to be the same')
    }}
  
  # data.table variables
  prob = method = adj.p.value = time = NULL
  simobject_list = sce_list
  subdir = paste0(save_dir,'/',pattern_name,'/')
  resultdir <- paste0(subdir,'probs_',spatial_probs[1],'_',spatial_probs[2],default,'/')
  ...
  for(rep_i in 1:reps) {
    ...
    if(file.exists(paste0(resultdir, save_name,'_',spatial_probs[1],'_',spatial_probs[2],'_final_object.rda'))) {
      print("Simulation dataset has been created. 
      ...
      if(exists('final_object1')) 
        final_object <-  final_object1
      else 
        final_object <- combined_object
    }else{
      final_object = CombineSimulatedObject(sce_list = sce_list,
                                            coordinate_name = coordinate_name,
                                            pattern_name = pattern_name,
                                            SvGene = SvGene,
                                            sample_names = sample_names,
    ...
    JointClus_BayesSpace(final_object, q = 2, path,sample_names)
